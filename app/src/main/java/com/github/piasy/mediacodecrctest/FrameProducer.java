        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mDecoder.configure(videoFormat, mHubSurface, null, 0);//配置输出缓存为mHubSurface
...
//                    Log.i(TAG, "sleep " + sleepTime);
    ...
    private boolean produceOneFrame() {//按照既定解码帧率一帧一帧的进行解码
        int decoderInputBufIndex = mDecoder.dequeueInputBuffer(TIMEOUT_US);//请求一个输入缓存index，timeoutUs代表等待时间，设置为-1代表无限等待
        if (decoderInputBufIndex < 0) {
            Log.i(TAG, "decoder input buffer not available");
            return true;
        }
        ByteBuffer inputBuf = mDecoder.getInputBuffer(decoderInputBufIndex);//获取输入缓存
        if (inputBuf == null) {
        ...
        int chunkSize = mExtractor.readSampleData(inputBuf, 0);//从视频文件读取数据放入输入缓存中
        if (chunkSize < 0) {//数据读取完毕，结束解码
...
//        Log.i(TAG, "extract frame: %d %d", chunkSize, presentationTimeUs);
        //调用queueInputBuffer(…)把缓存数据入队，chunkSize为需要解码的数据大小
        mDecoder.queueInputBuffer(decoderInputBufIndex, 0, chunkSize, presentationTimeUs, 0);
        mExtractor.advance();

        while (true) {//将解码数据输出到mHubSurface
            //请求一个输出缓存索引，这里需要传入一个BufferInfo对象用于存储ByteBuffer的信息
