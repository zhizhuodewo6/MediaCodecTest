...
public class EncoderWrapper extends MediaCodec.Callback {
    ...
    private int mCurrentBr;//当前码率
    private final Runnable mRcRunnable = new Runnable() {
        @Override
        public void run() {
            if (mCurrentBr > mConfig.initBr() * 2) {//当前码率大于初设码率
                mRcSign = -1;
            } else if (mCurrentBr < mConfig.initBr()) {//当期码率小于初设码率
                mRcSign = 1;
            }
    ...
    private RcTest.EncodeCallBack mEncodeCallBack;
    public void start(final EglBase eglBase, RcTest.EncodeCallBack encodeCallBack) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    encodeFormat.setInteger(MediaFormat.KEY_BITRATE_MODE, mConfig.brMode());//MediaCodecInfo.EncoderCapabilities.BITRATE_MODE_VBR
    ...
    private void startOutputThread() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (mRunning) {
                    int index = mEncoder.dequeueOutputBuffer(mBufferInfo, TIMEOUT_US);//获取输出缓冲指针
                    if (index < 0) {
                        continue;
                    }

                    ByteBuffer buffer = mEncoder.getOutputBuffer(index);//获取输出数据
                    if (buffer != null) {
    ...
    public void stop() {
        ...
        Log.e("haha","结束了");
        mEncodeCallBack.callBack(mCurrentBr);
    ...
    private void reportEncodedImage(final MediaCodec.BufferInfo info, final ByteBuffer buffer) {
        ...
        if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) == 0) {//没有结束编码
//            Log.i(TAG, "reportEncodedImage %d %d %d",
//                    info.flags & MediaCodec.BUFFER_FLAG_KEY_FRAME, info.size,
//                    info.presentationTimeUs);
            if (System.currentTimeMillis() - mLastResetBitsTime > 1000) {//确保一帧一个
                mNotifier.reportBr(mOutputBits);//返回当前编码后一帧码率
                mOutputBits = 0;
                mLastResetBitsTime = System.currentTimeMillis();
            }
